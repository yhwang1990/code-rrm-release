...
def GetWeightsForFiveDimension(seed, k):
    d = 5
    ...
    for i in range(m):
        ...
        for j in range(m):
            ...
            for t in range(m):
                axis4 = gap / 2.0
                for q in range(m):
                    tempCenterPoints.append([axis1, axis2, axis3, axis4])
                    axis4 += gap
                axis3 += gap
            axis2 += gap
        axis1 += gap
    ...
    for item in tempCenterPoints:
        tempNum = math.sqrt(item[0] * item[0] + item[1] * item[1] + item[2] * item[2] + item[3] * item[3] + 1.0)
    ...
    for i in range(residueNum):
        ...
        weights.append([tempMatrix[i, 0] / tempNum, tempMatrix[i, 1] / tempNum, tempMatrix[i, 2] / tempNum,
                        tempMatrix[i, 3] / tempNum, tempMatrix[i, 4] / tempNum])
    return weights
...
def RRMS(seed, dim, k, r, data, attr, groups):
    weights = GetWeightsForFiveDimension(seed, k)
    if len(weights) == 0:
        return []
    sol = []
