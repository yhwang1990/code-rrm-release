...
import max_cover_RRMS_3D
import max_cover_Polytope_3D
...
if __name__ == "__main__":
    ...
    data, attr, groups = readData('./data/email-Eu-core.txt', './data/email-Eu-core_3.txt', 3, is_directed=True)
    rows_list = []
    ratio, _ = max_cover_RRMS_3D.SingleObj(r, data, attr, groups)
    rows_list.append({'data': 'email-Eu-core', 'alg': 'SingleObj', 'k': 3, 'd': 3, 'avg': ratio, 'std': 0, 'min': ratio, 'max': ratio})
    vals_k = list(range(1, 20))
    for ki in range(20, 51, 5):
        vals_k.append(ki)
    for k in vals_k:
        print('****************** k = %d *********************' % k)
        ratio, _ = max_cover_Polytope_3D.Do(k, r, data, attr, groups)
        rows_list.append({'data': 'email-Eu-core', 'alg': 'Polytope', 'k': k, 'd': 3, 'avg': ratio, 'std': 0, 'min': ratio, 'max': ratio})
        ...
        for seed in range(10):
            ...
            ratio, _ = max_cover_RRMS_3D.RRMS_Origin(seed, k, r, data, attr, groups)
            mrr_rrms[seed] = ratio
            ratio, _ = max_cover_RRMS_3D.RRMS_Star(seed, k, r, data, attr, groups)
            mrr_rrms_star[seed] = ratio
        rows_list.append({'data': 'email-Eu-core', 'alg': 'HS-RRM', 'k': k, 'd': 3, 'avg': np.average(mrr_hs_rrm),
                          'std': np.std(mrr_hs_rrm), 'min': np.min(mrr_hs_rrm), 'max': np.max(mrr_hs_rrm)})
        rows_list.append({'data': 'email-Eu-core', 'alg': 'RRMS', 'k': k, 'd': 3, 'avg': np.average(mrr_rrms),
                          'std': np.std(mrr_rrms), 'min': np.min(mrr_rrms), 'max': np.max(mrr_rrms)})
        rows_list.append({'data': 'email-Eu-core', 'alg': 'RRMS-Star', 'k': k, 'd': 3, 'avg': np.average(mrr_rrms_star),
                          'std': np.std(mrr_rrms_star), 'min': np.min(mrr_rrms_star), 'max': np.max(mrr_rrms_star)})
    df = pd.DataFrame(rows_list)
    df.to_csv('./max_cover_d3.csv')
